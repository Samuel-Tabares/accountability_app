...
export async function POST(request: NextRequest) {
  ...
  const { error: authError } = await auth.adminClient.auth.admin.updateUserById(auth.userId, { password });
  if (authError) {
  ...
  const { error: profileError } = await auth.adminClient
    .from("profiles")
    .update({ must_change_password: false, password_updated_at: new Date().toISOString() })
    .eq("id", auth.userId);
  ...
  if (jsonMode) return jsonResponse(true, "Contraseña actualizada.", 200, { redirectTo });
